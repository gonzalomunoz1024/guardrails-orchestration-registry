 ...
 *
 * Usage:
 *   node serve.cjs --api-url http://localhost:8181 --port 3000
 *
 * Options:
 *   --api-url, -a   Backend API base URL (default: empty; all endpoints live under /v1/utilities)
...
function proxyToBackend(backendUrl, targetPath, req, res) {
  ...
  // e.g., backendUrl = "http://localhost:8080", targetPath = "/v1/utilities/registry/guardrails"
  // result = "/v1/utilities/registry/guardrails"
...
Options:
  --api-url, -a <url>   Backend API base URL (default: empty)
...
Examples:
  node serve.cjs --api-url http://localhost:8181
  node serve.cjs -a http://backend:8181 -p 8080
`);
      process.exit(0);
    }
...
function main() {
  ...
  // When proxying (apiUrl is a full URL), frontend uses '' (bare absolute paths
  // under /v1/utilities/...) and we proxy /v1/ requests to the backend.
  const frontendApiUrl = config.apiUrl.startsWith('http') ? '' : config.apiUrl;
...
`;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'OPTIONS' && urlPath.startsWith('/v1/')) {
      res.writeHead(204, {
        'Access-Control-Allow-Origin': '*',
        'Access-Control-Allow-Methods': 'GET, POST, PUT, PATCH, DELETE, OPTIONS',
        'Access-Control-Allow-Headers': 'Content-Type, Authorization',
    ...
    // Frontend uses bare /v1/utilities/... paths, so forward them as-is.
    if (urlPath.startsWith('/v1/') && config.apiUrl.startsWith('http')) {
      const queryString = req.url.includes('?') ? req.url.substring(req.url.indexOf('?')) : '';
      const targetPath = urlPath + queryString;
      proxyToBackend(config.apiUrl, targetPath, req, res);
      return;
    }

    // Health check endpoint (local)
    if (urlPath === '/health' && !config.apiUrl.startsWith('http')) {
